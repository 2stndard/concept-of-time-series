...
# ydya()를 사용하여 1월 1일부터 오늘까지의 날짜수 계산
paste0('1월 1일부터 오늘까지 ', yday(now.date), '일 지났습니다')
# qday()를 사용하여 분기 시작일로부터 오늘까지의 날짜수 계산
...
# 2020-02-01부터 한달 후(period)는 21년 3월 1일
as.Date('2020-02-01') + months(1)  
# 2020-02-01부터 한달 후(duration)는 30일 후인 20년 3월 2일
as.Date('2020-02-01') + dmonths(1)  
# 2021-02-01부터 한달 후(period)는 21년 3월 1일
as.Date('2021-02-01') + months(1)  
# 2020-01-01부터 한달 후(duration)는 30일 후인 3월 2일
...
# 2020-10-01에서부터 2020-10-10까지 데이터 filtering
covid19.xts['2020-10-01/2020-10-10']  
# 2021-02-05일부터 끝까지 데이터 filtering
...
students.xts[, c('학생수계', '증감', '증감률')] %>% head(10)
plot.xts(students.xts[, '증감률'], main = '전년 대비 학생수 증감률')
