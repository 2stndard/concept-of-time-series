...
시계열 데이터는 일반적으로 가로축(X axis)에는 시간 인덱스가 위치하고 세로축(Y axis)에 값이 표현되는 라인 그래프가 일반적으로 많이 사용된다.이러한 그래프의 형태는 시간의 흐름에 따라 발생하는 추세(Trend), 계절성(Seasonality) 등의 시계열 특성을 표현하고 파악하는데 효율적이다. 시계열 데이터 분석에는 시계열 데이터의 직접적인 라인 그래프 외에도 자기 상관 함수(ACF : AutoCorelation Function), 편자기상관함수(PACF : Partial AutoCorelation Function) 등의 다양한 그래프들이 존재하나 이 장에서는 data.frame, ts, xts, tsibble 등 시계열 객체별로 plotting을 작성하는 방법을 설명한다.
...
```{r ggplot_covid}
ggplot(data = wide.covid19.by.age, aes(x = as.factor(date), y = `0-9세`)) +
  geom_line(aes(group = 1)) + 
  geom_point(shape = 'circle') +
  labs(title = '일별 확진자수(0-9세)', x = '시간') +
  scale_y_continuous(labels = scales::number_format(big.mark = ',')) +   
  scale_x_discrete(breaks = c('2020-04-10', '2020-05-01', '2020-06-02', '2020-07-01', '2020-08-01', '2020-09-01', '2020-10-01', '2020-11-01', '2020-12-01', '2021-01-01', '2021-02-01')) +
  theme(axis.text.x=element_text(angle=90,hjust=1))
```
...
```{r xts_employees}
plot.xts(employees.xts[, 1], main = '월별 취업자수', xlab = '연도',  ylab = '취업자수')
```

```{r xts_covid19}
plot.xts(wide.covid19.by.age.xts, main = '일별 확진자수', xlab = '날짜',  ylab = '확진자수')
...
```{r forecast_autoplot, message=FALSE, warning=FALSE}
library(forecast)
...
tsibble 객체는 forecast 패키지에서 제공하는 autoplot() 함수를 통해 ploting 할 수 있다. 다만 tsibble 객체를 ploting 하기 위해서는 feasts 패키지를 설치 후 loading 시켜야 한다.
...
feasts 패키지에서는 ggplot2와 유사한 계절성 plot 함수인 gg_season() 함수와 gg_subseries() 함수를 제공한다.
