...
```{r fable_students5}
best.model.fable.students <- model.fable.studetns %>% select(prophet, arima)
```

```{r fable_students6}
best.model.fable.students %>% forecast(h = 10) %>% autoplot(students.tsibble, level = NULL) + autolayer(fitted(best.model.fable.students)) + geom_point(aes(x = 연도, y = 학생수계))
```
