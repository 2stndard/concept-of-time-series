...
아래는 총 취업자수에 대한 홀트윈터 모델 plot이다. 덧셈방법과 곱셈방법의 두가지 plot을 보여주고 있다.
...
autoplot(employees.ts[,2]) + 
  ...
  labs(title = '취업자수 holt winter 지수평활모델', x = '연도', y = '취업자수')
```
...
```{r ets2}
ets(students.ts[,2]) %>% autoplot()  ### 전체 학생수에 대한 ets 모델 ploting
```
...
사실 ARIMA 모델은 ARMA(AutoRegression Moving Average)을 기반으로 한 모델이지만 ARMA 모델은 정상성 시계열에 한한 모델이기 때문에 비정상성 데이터를 정상화하는 단계를 포함하여 ARIMA 모델로 구현된다.[^forecasting-ii-10]
...
ARIMA 모델에서 이동 평균을 사용할 때 주의해야 할점은 이동 평균 모델의 계수를 적용하는 독립 변수가 오차항이라는 점이다. 앞선 자기회귀모델에서는 자기회귀계수를 자기자신의 lag 차수값에 적용하였지만 이동 평균 모델은 이동평균 계수를 오차항에 적용시킨다. (이해가 어렵지만 원리를 알고싶다면 참고문헌[^forecasting-ii-11]을 참조하라)
...
urca::ur.kpss(arima011) %>% urca::summary()    ### kpss 테스트를 통해 생성된 데이터가 정상성인지 테스트 - 0.05보다 작으므로 정상성, 차분 필요
...
위의 예와 같이 p와 q가 모두 0보다 큰 경우는 ACF와 PACF plot으로 모델을 결정하는 것이 적절하지 않을 수 있다. [^forecasting-ii-12] 이 경우에는 여러가지 경우수를 설정하고 AIC, BIC, RMSE 등의 성능 분석 수치를 통해 가장 좋은 모델을 선정해야 한다. 이와 관련된 내용은 다음 장에서 다루겠다.
...
ndiffs(covid19.ts[,2], test = 'kpss')   ### 비정상성을 제거하기 위해 필요한 차분수가 1
diff(covid19.ts[,2]) %>% ggtsdisplay()  ### ACF, PACF 모두 절단(Cut off)이므로 ARMA(p, q) 모델 
auto.arima(covid19.ts[,2]) ### 0-9세 코로나 확진자수의 ARIMA모형은 ARIMA(2, 1, 1)으로 선정됨
...
autoplot(qtr.employees.ts[,3]) + 
  ...
  labs(title = '분기별 취업자수에 대한 ARIMA(0,1,0)(0,1,0)[4]와 ARIMA(0,1,0)(1,1,0)[4]', x = '연도', y = '취업자수', color = '모델')
...
autoplot(employees.ts[,2]) + 
  ...
  labs(title = '월별 취업자수에 대한 ARIMA(0,1,1)(1,1,0)[12]와 ARIMA(0,1,0)(0,1,1)[12]', x = '연도', y = '취업자수', color = '모델')
...
```{r TBATS}
covid19.ts[,2] %>% tbats() %>% forecast() %>% autoplot()   ### 코로나 확진자 데이터에 대한 tBats 모델
```
...
students.ts[,2] %>% nnetar() %>% forecast(PI = TRUE) %>% autoplot()
...
covid19.ts[,2] %>% nnetar() %>% forecast(h = 100, PI = TRUE) %>% autoplot()
```
