...
lubridate::as.period(int)   # 년월일 형태로 기간 반환
lubridate::as.duration(int)  # 경과 초 형태로 기간 반환
...
cbind(students %>% filter(지역규모 == '계') %>% select(연도, 학생수계),
...
plot.xts(students.total.xts[, '증감율'], main = '전년대비 학생수 증감률')
...
wide.covid19.by.age %>%
  mutate(month = lubridate::month(wide.covid19.by.age$date)) %>%
  ggplot(aes(as.factor(month), `0-9세`)) + 
  geom_boxplot() + 
  geom_jitter(alpha = 0.2)

wide.covid19.by.age %>%
  mutate(wday = lubridate::wday(wide.covid19.by.age$date, label = TRUE)) %>%
